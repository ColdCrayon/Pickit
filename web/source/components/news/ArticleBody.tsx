  ...
  return (
    <div className="prose prose-invert max-w-none">
      <ReactMarkdown
        ...
        components={{
          img: (props) => <img loading="lazy" className="rounded-md my-4" {...props} />,
          a: (props) => <a className="text-yellow-400 underline" {...props} />,
          h1: ({ node, ...p }) => <h2 className="mt-8 mb-4 text-2xl font-semibold" {...p} />,
          h2: ({ node, ...p }) => <h3 className="mt-6 mb-3 text-xl font-semibold" {...p} />,
          code: ({ node, ...p }) =>
            (node && (node as any).inline) ? (
              <code className="px-1 py-0.5 rounded bg-neutral-800" {...p} />
            ) : (
              <code className="block p-3 rounded bg-neutral-900 overflow-x-auto" {...p} />
            ),
        }}
      >
        {markdown}
